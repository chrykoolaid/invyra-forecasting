...
def _record(history_id: str, item_id: str, location_id: str, created_at_utc: str) -> ForecastHistoryRecord:
...
def test_breakdown_route_is_get_only() -> None:
    matching = [
        route
        for route in router.routes
        if route.path == "/v1/intelligence/operational/portfolio/breakdown"
    ]
...
def test_breakdown_endpoint_reads_durable_history(tmp_path, monkeypatch) -> None:
    history_dir = tmp_path / "history"
    monkeypatch.setenv("INVYRA_FORECAST_HISTORY_DIR", str(history_dir))
    repository = FileForecastHistoryRepository(history_dir)
    repository.append(_record("h1", "ITEM-1", "LOC-1", "2026-07-01T00:00:00+00:00"))
    repository.append(_record("h2", "ITEM-1", "LOC-2", "2026-07-02T00:00:00+00:00"))

    response = client.get(
        "/v1/intelligence/operational/portfolio/breakdown",
        params={"as_of_utc": "2026-07-10T00:00:00+00:00"},
    )

    assert response.status_code == 200
    payload = response.json()
    assert payload["resource"] == "operational_forecast_portfolio_breakdown"
    ...
    assert len(data["item_locations"]) == 2
    assert data["items"][0]["forecast_record_count"] == 2


def test_breakdown_endpoint_returns_controlled_bad_request(tmp_path, monkeypatch) -> None:
